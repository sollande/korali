...
def model(p):
 ...
 DestinationFolderName = "Results_sample" + str(p["Sample Id"]).zfill(2)+'/'
 ...
 with open(configFile, 'w') as f:
  ...
  f.write('set string grid N\n')
  f.write('set string gridF N\n')
 ...
 fig, ax = plt.subplots()
 ax.plot(t,vmi,c='r',label='Reference profile')
 ax.plot(v_plot.t,v_plot.vx,c='b',label='Estimated profile')
 ax.set(title='Estimated vs Reference velocity profiles, Obj_f ='+str(-np.sqrt(np.sum((vm.values-vmi)**2))))
 ...
 p["F(x)"] = -np.sqrt(np.sum((vm.values-vmi)**2))
