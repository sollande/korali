...
def model(p):
 ...
 FFTref = np.fft.fft(vmref)
 FFT = np.fft.fft(vmi)

 freq = np.fft.fftfreq(tref.shape[-1])
 ...
 ax[1].plot(freq,np.abs(FFTref),c='r',label='Reference FFT Magnitude')
 ax[1].plot(freq,np.abs(FFT),c='b',label='Estimated FFT Magnitude')
 ax[1].set_xlabel('Sampling freq')
 ax[1].set_ylabel('Magnitude |FFT(f)|')
 ax[1].legend()

 ax[2].plot(freq,np.angle(FFTref),c='r',label='Reference FFT Phase')
 ax[2].plot(freq,np.angle(FFT),c='b',label='Estimated FFT Phase')
 ax[2].set_xlabel('Sampling freq')
 ax[2].set_ylabel('Phase')
 ax[2].legend()

 fig.set_size_inches(15,5)
 fig.suptitle('Estimated vs Reference velocity profiles, Obj_f ='+str(-np.sqrt(np.sum((np.abs(FFTref)-np.abs(FFT))**2))-np.sqrt(np.sum((np.angle(FFTref)-np.angle(FFT))**2))), fontsize=15)
 ...
 p["F(x)"] = -np.sqrt(np.sum((np.abs(FFTref)-np.abs(FFT))**2))-np.sqrt(np.sum((np.angle(FFTref)-np.angle(FFT))**2))
