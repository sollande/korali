...
def model(p):
 ...
 aref = (vs.values[1:]-vs.values[:-1])/(ts.values[1:]-ts.values[:-1])
 a = (vsi[1:]-vsi[:-1])/(ts.values[1:]-ts.values[:-1])
 ay = (vsi[1:]-vsi[:-1])
 ...
 ax[0].plot(t,vmi,c='r',lw=2,label='Reference profile')
 ax[0].plot(t,vm,c='g',lw=2,label='Estimated profile')
 ax[0].plot(v_plot.t,v_plot.vx,'b--',lw=1,label='Full 10s Estimated profile')
 ax[0].set(title='Estimated vs Reference velocity profiles, Obj_f ='+str(-np.sqrt(np.sum((vm.values-vmi)**2))))
 ax[0].set_xlabel('Time [s]')
 ax[0].set_ylabel('Horizontal velocity [m/s]')
 ax[0].legend()
 
 ax[1].quiver(sref.t.values[:-1],np.linspace(0,0,len(sref)-1),(sref.t.values[1:]-sref.t.values[:-1]),(sref.vx.values[1:]-sref.vx.values[:-1])*10,color='r',label='Reference')
 ax[1].quiver(sref.t.values[:-1],np.linspace(0,0,len(sref)-1),(sref.t.values[1:]-sref.t.values[:-1]),(vsi[1:]-vsi[:-1])*10,color='b',label='Estimated')
 ax[1].set(title='Slopes, Obj_f ='+str(-np.sqrt(np.sum((aref-a)**2))))
 ax[1].set_xlabel('Time [s]')
 ax[1].set_ylabel('Slope')
 ax[1].set_ylim(-1.5*np.abs(np.max(ay)),1.5*np.abs(np.max(ay)))
 ax[1].legend()
 
 fig.suptitle('Total Cost (fit + 2 x slope) = '+str(-(np.sqrt(np.sum((vm.values-vmi)**2))+2*np.sqrt(np.sum((aref-a)**2)))), fontsize=15)
 ...
 with open('../COSTS.txt', 'a') as g:
  g.write('GEN :'+str(p["Current Generation"])+' SAMPLE :'+str(p["Sample Id"])+'\n')
  g.write('COST : '+str(-(np.sqrt(np.sum((vm.values-vmi)**2))+2*np.sqrt(np.sum((aref-a)**2)))))
  g.write('\n')


 # Assigning objective function value
 p["F(x)"] = -(np.sqrt(np.sum((vm.values-vmi)**2))+2*np.sqrt(np.sum((aref-a)**2)))
