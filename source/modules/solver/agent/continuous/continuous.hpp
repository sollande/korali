...
class Continuous : public Agent
{
  ...
  */
  ...
  /**
   * @brief Calculates the gradient of the action wrt to the policy parameter of the current distribution.
   * @param action The action taken by the agent in the given experience
   * @param curPolicy The current policy for the given state
   * @param oldPolicy The policy for the given state used at the time the action was performed
   * @return gradient of action wrt curParamsOne and curParamsTwo
   */
  std::vector<std::vector<float>> calculateActionPolicyParameterGradient(const std::vector<float> &action, const policy_t &curPolicy, const policy_t &oldPolicy);
